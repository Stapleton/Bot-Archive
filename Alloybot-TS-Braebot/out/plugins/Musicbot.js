...
class MBCommands {
    ...
    static async Play(Message) {
        try {
            ...
            Connection.playStream(YTStream, { passes: 4, volume: Number("0.01"), bitrate: "auto" });
    ...
    static async Volume(Volume) {
        ...
        if (Volume == "current") {
            ...
            if (VolumeInt > 0 || VolumeInt < -50) {
                Values_1.default.MusicChannel.send(`Can't really get any spicier... (Volume Cap is -50dB to 0dB)`);
                return;
            }
            else {
